...
func (s *SQLiteStorage) SaveJob(j *job.Job) error {
	query := `
	INSERT INTO jobs (id, command, state, attempts, max_retries, priority, timeout_seconds, created_at, updated_at, next_retry_at, run_at, worker_id, error, output, execution_time)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	var nextRetryAt, runAt interface{}
	...
	if err != nil {
		return fmt.Errorf("failed tpackage storage
...
import (
	...
	"github.com/yourusername/queuectl/internal/job"
)
